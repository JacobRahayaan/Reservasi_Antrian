<header class="sticky top-0 z-40 border-b border-pln-slate-200 bg-white print:hidden">
    <nav class="mx-auto flex h-16 max-w-6xl items-center justify-between px-4 sm:px-6 lg:px-8" aria-label="Navigasi utama">
        ...
        <div class="hidden items-center gap-3 md:flex">
            <x-button href="{{ route('reservasi.cek-status.form') }}" variant="ghost" size="sm" class="hidden sm:inline-flex">
                Cek Status
            </x-button>
        </div>
        <button
    ...
    <div id="mobile-nav" class="hidden border-t border-pln-slate-200 md:hidden">
        <div class="space-y-1 px-4 py-3">
            ...
            <a href="{{ route('reservasi.cek-status.form') }}" class="block rounded-lg px-3 py-2 text-sm font-medium text-pln-slate-600 hover:bg-pln-slate-100">Cek Status</a>
        </div>
    </div>
</header>