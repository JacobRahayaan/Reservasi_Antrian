            ...
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p class="mt-1 text-sm text-pln-slate-600">
                        Ringkasan slot dan kuota reservasi per tanggal. Klik tanggal untuk melihat detail jadwal.
                    </p>
                </div>

                <div class="flex items-center gap-2">
                    <a href="{{ route('admin.kalender.index', ['bulan' => $bulanSebelumnya]) }}" class="flex h-9 w-9 items-center justify-center rounded-lg border border-pln-slate-200 text-pln-slate-600 transition hover:bg-pln-slate-100" aria-label="Bulan sebelumnya">
                    ...
                    <a href="{{ route('admin.kalender.index', ['bulan' => $bulanBerikutnya]) }}" class="flex h-9 w-9 items-center justify-center rounded-lg border border-pln-slate-200 text-pln-slate-600 transition hover:bg-pln-slate-100" aria-label="Bulan berikutnya">
                        <x-icon name="chevron-right" class="h-4 w-4" />
                    </a>
                    <a href="{{ route('admin.kalender.index') }}" class="rounded-lg border border-pln-slate-300 px-3 py-2 text-sm font-medium text-pln-slate-700 transition hover:bg-pln-slate-50">
                        Bulan Ini
                    </a>
                </div>
            </div>
